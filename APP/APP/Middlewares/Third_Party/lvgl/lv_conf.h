...
/* ---------------- D00 工程适配 ---------------- */
#include <stdint.h>
/* ARMCC 5.06 的 <stdint.h> 在 C99 模式下偶发不导出 UINT32_MAX，
 * LVGL v8 多处直接引用；此处兜底定义（已有定义则跳过，无副作用）。 */
#ifndef UINT32_MAX
...
#if LV_TICK_CUSTOM
    /* D00：LVGL 的 lv_tick_get() 直接查询 HAL SysTick（1ms 精度） */
...
#define LV_ASSERT_HANDLER while(1);   /*Halt by default：断言触发挂起保留现场（DAP 可取证）*/
