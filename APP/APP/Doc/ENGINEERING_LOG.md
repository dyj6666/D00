...
### 10.48 MPU6050 总线钳位根治 + OTA 写入稳定性（重点问题）

**现象**：MPU6050 已接线但检测不到（faults=1，启动即 FAIL）；`mpu test` 输出
全 0 流；DAP 实测 **GPIOB IDR：SCL=1/SDA=0（SDA 被拉低）+ I2C1 SR2 BUSY**。

#### 10.48.1 排查过程

1. 驱动/I2C1 配置检查（PB6/7 AF4 开漏上拉、400kHz、0x68→0x69 双地址）——正确；
2. DAP 实测总线：SDA=0 + I2C BUSY——**定位为 I2C 从机钳位**（从机等待时钟，
   标准释放 = 主机 ≥9 个 SCL 脉冲）；
3. DAP 手动 toggle SCL 9 次 → SDA 释放（=1）→ 复位后 **MPU 恢复（ready=1，
   重力 1.12g Z 轴、温度 29°C）**——实锤钳位是唯一根因；
4. 排查钳位来源：驱动失败路径只 DeInit/Init（**不产生时钟，无法释放从机**），
   ImuSvc 失败后死循环不重试 → 一次钳位即永久 FAIL；DAP 调试（halt 期间
   若 I2C 传输被截断）正是历史反复触发钳位的来源。
5. **附带发现**：MPU 恢复后 OTA 写入随机失败（失败点数据已写入但报错）——
   SPI 字节级 200µs 超时在系统负载（I2C/ETH 中断）下偶发误判；
   失败点提前随残留数据累积（未擦净区域 AND）。

#### 10.48.2 修复

1. **BSP_I2C1_BusRelease()**：停 I2C → PB6/7 转开漏 GPIO → 产生 ≥9 个 SCL
   脉冲释放从机钳位 → HAL_I2C_Init 恢复 AF 开漏上拉；
2. **bsp_mpu6050**：reg_op 失败路径与 Init 探测失败均调用 BusRelease；
3. **imu_svc**：初始化失败改为每 2s 周期重试（自愈，记录 fault_count）；
4. **bsp_w25q128**：SPI 字节级超时 200µs → 5ms（负载下写入稳定）。

#### 10.48.3 验证

- **手动钳位→9 时钟释放**：MPU 从 FAIL 恢复 ready（首轮实证）；
- **v209 运行时钳位自愈**：DAP 拉低 SDA（不复位）→ 3s 内 MPU 自动恢复
  （ready=1、faults=0、采样连续）——BusRelease 闭环生效；
- **OTA 稳定性**：v207 期间多次随机失败 → v208（5ms 超时）→ **v209 一次通过**；
- 双构建 0 错 0 警告，MPU 数据真实（重力/温度/陀螺仪）。

**经验沉淀**：①I2C 从机钳位必须用 ≥9 个 SCL 脉冲释放（DeInit/Init 无效）；
...
- **排查**：
  1. 隔离测试（LA 回退/栈 8KB/堆 52KB）均 FAIL——与存储优化无关；
  2. I2C 错误码为 AF（地址无应答）；**总线扫描 0x08~0x77 无任何设备 ACK**；
  3. **发现并修复两个真实软件缺陷**：
     - HAL_I2C_MspInit 在部分构建产物中 GPIO AF 配置未生效（PB6/7 保持 AF0）→ 寄存器级强制 AF4；
     - HAL_I2C_Init 的 CCR/TRISE 配置异常（实测 CCR=0x0D/0x8023，非 400kHz 值）→ 每次传输前寄存器级兜底写入 0x8035/43；
  4. 修复后 400kHz/100kHz 均测试——**MPU 仍无 ACK**，判定硬件层面未响应。
...
### 10.55 LVGL 性能拉满与两起硬件级疑难（2026-08-15，重点问题）
**背景**：为"顶级 GUI"打基础，硬件端 LVGL 跑分 + 系统优化。新增 `gui bench` 命令
（内存带宽/LCD 原始层/LVGL 场景帧时间/flush 吞吐分解）、LCD 读回自检 `lcd selftest`。

**关键问题一：DMA 异步 flush 引发 FSMC 总线级死锁（废弃该方案）**
- 现象：DMA2 Stream1/Stream4 M2P 直写 FSMC LCD 后，GUI 任务卡死、ETH 无响应
  （ping 不通、TCP :9020 连接超时），基准命令无输出。
- DAP 取证：PC 两次采样均停在 `0x0802C2FC`，指令解码 `LDR R2,[R1]; LSLS R2,#31;
  BNE -8` = 轮询某寄存器 bit0（DMA CR EN 位）永不置零——DMA 传输永不完成；
  且 DMA 占用 FSMC 总线期间 CPU 的窗口命令写互锁，连带 ETH 服务无响应。
- 根因：F407 FSMC 上 DMA 与 CPU 并发访问互锁（FSMC 控制器无超时自愈）。
- 解决：废弃 DMA 异步 flush，回退 CPU 同步 flush（性能损失可接受）。
- 验证：回退后 ping 恢复、基准跑通。

**关键问题二：ST7789 竖屏窗口写错位（"斜"/镜像花屏根治）**
- 现象：LVGL 文字与图形整体"变斜"、多行块第二行起内容错位/残留。
- 排查（读回自检证据链，>5 步）：
  1. 渐变行写读回：首行首列对、其余错 → 疑窗口/行宽；
  2. 单点写读 5/5 全对 → 写读基本路径正确、连续写特有；
  3. 单行 240×1 窗口连续写读回全对、100×1@50 全对 → 单行窗口正确；
  4. 多行 60×30 窗口连续写：第一行对、第二行起读回清屏黑 → **多行窗口写不进后续行**；
  5. 单点窗口（set_cursor）写 60 像素：仅第一列收到 → **单点窗口只收 1 像素（超窗丢弃）**；
  6. 3 行不同色网格：每行仅首列有值 → set_cursor 行切换正常、列不扩展。
- 根因：ST7789 竖屏（MV）下 RAMWR 多行窗口行递增异常 + 单点窗口超窗丢弃——
  唯一正确路径是"每行设单行多列窗口 + 行数据连续写"。历史 lcd_fill 连续写全屏
  清黑看不出错误，LVGL 彩色内容首次暴露（历史遗留 bug）。
- 解决：BSP_LCD_WritePixels 与 lcd_fill 均改为逐行 `lcd_set_window(x, y+row, w, 1)` 写。
- 验证：lcd selftest 全绿（win 4 角/grid 3×3/dot 5 点/rowmap 两窗全对）。

**性能数据（168MHz，240x320，优化前后对比）**：
| 场景 | 基线(外SRAM 40行/30ms节流) | 最终(主SRAM 12行/16ms/逐行窗) |
|---|---|---|
| 全屏纯填充 | 33fps(节流)/64fps(直测) | 58 fps |
| 色带全屏 | 47 fps | 44 fps |
| 骨架 UI | 18 fps | 17 fps |
| 局部动画 | 347 fps | 305 fps |
优化项：绘制缓冲迁主 SRAM（读 18→66MB/s）、REFR 30→16ms、LVGL 堆/层缓冲
评估、flush 吞吐 26MPix/s（LCD 写上限）；逐行窗口为 ST7789 硬件行为必要代价
（吞吐 26→17MPix/s）。UI 场景瓶颈在对象遍历+外部 SRAM 堆读（渲染占 90%），
留给界面打造阶段按"对象精简/样式缓存"优化。
...
### 10.57 DMA M2M 完全可用——10.56 结论更正（2026-08-15，重点问题）
**背景**：用户连续质疑"DMA M2M 不能用？"，复盘发现 10.56 的终审结论错误。

**被推翻的错误结论**：10.56 判定"DMA2 M2M 不启动"——实为两个配置错误叠加：
1. **DIR 位编码误解**：F407 的 DMA_SxCR DIR 是 2 比特编码
   （00=P2M、01=M2P、**10=M2M**）；此前一律用 DIR_0(=01=M2P) 做 M2M 测试，
   DMA 实际在等待不存在的外设请求，EN 保持/NDTR 不动。
   （HAL 头早已明示：DMA_MEMORY_TO_MEMORY = DMA_SxCR_DIR_1）
2. **源/目的寄存器接反**：M2M 数据流为 **PAR=源、M0AR=目的**（PINC 递增源、
   MINC 固定目的）；此前按 M0AR=源配置，数据搬反方向。

**DAP 证据（debug 单会话，零烧录）**：
- 正确配置（DIR_1 + PAR=src + M0AR=dst + PINC + 16bit）后：
  NDTR 8→0、EN 自动清、dst 完整收到 0x80018002/0x80048003 ✓✓
- 交换 PAR/M0AR 对照：src 被清零 → 证实方向为 PAR→M0AR。

**DMA 直写 LCD 的最终工程结论**：可用但无收益。
- 实机 BSP_LCD_WritePixelsDma（同步等待版）：flush 10 MPix/s vs CPU 写
  18 MPix/s——逐行窗口（ST7789 硬件限制）下每行 DMA 配置开销（~15us）
  大于 CPU 搬运（~7us），同步等待净损失；
- 异步版可将配置开销与渲染重叠，但收益仅 ~10% 且复杂度高；
- **CPU 同步 flush 保持为最优路径**；BSP_LCD_WritePixelsDma 保留为
  备选/诊断接口（注释注明实测数据）。
...
### 13.1 主 RAM 布局审计：bss 与主栈 192B 重叠（2026-08，重点问题）

**现象**：上板前"确保各内存区可用"静态审计（GCC APP.map）发现：
主 RAM `.bss` 顶 = `0x2001FCC0`，主栈顶 `_estack` = `0x2001FC00`——
**bss 越过栈顶 192B**。满减栈从 `_estack` 向下，压栈首字节即写入
bss 顶部（libc `__sf` 文件结构/锁区），printf/锁可能偶发异常；
主 RAM 使用率 99.4%（128KB 仅剩 832B）。

**排查思路**：先确认不是链接失败（链接通过、无溢出报错），再用
`_ebss` 与 `_estack` 差量定位重叠量（-192B），最后从 map 统计
.bss 大户找可迁移缓冲——要求：纯 CPU 访问、无 DMA 依赖、互斥保护。

**被推翻的方向**：最初怀疑改 `_estack` 位置（去掉 1KB 安全余量）——
否决：注释明确"安全余量与 Keil 对齐"，且治标不治本（bss 仍顶到栈底）。

**根因**：新增 GUI 数据快照（1.2KB）曾使主 RAM 溢出 216B，当时把
GUI 快照迁入 CCM 解了链接溢出，但 bss 顶仍紧贴安全区（0x340 余量被
既有 .bss 吃尽后继续上探 192B），主栈无独立空间。

**解决方案**：`var_manager.c` 变量注册表 `registry[64]`（1KB，纯 CPU
访问、互斥保护、无 DMA）迁移 `.ccmram`——一处改动即把 bss 顶降到
`0x2001F8C0`，与主栈间距恢复 **832B**，顶部 1KB 安全预留完整。

**验证**：
- GCC 重链：bss 顶 `0x2001F8C0`，gap=832B ✅；CCM 余 3.8KB ✅；
- Keil AC5 增量：**0 Error / 0 Warning**（.ccmram 段 AC5 兼容，
  event_bus 池已有同机制先例）；
- 主 RAM 各区最终布局：.data 1.1KB / .bss 125KB / 主栈区 1.8KB /
  顶部 1KB 预留，无重叠无越界。

**经验沉淀**：
- "链接通过 ≠ 布局健康"：主栈与 .bss 的间距必须显式审计
