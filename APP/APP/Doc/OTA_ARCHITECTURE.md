...
0x08010000 +-------------------+  320KB 扇区4-6   RUN（APP 运行区）
           |       RUN         |
0x08060000 +-------------------+  256KB 扇区7-8   BACKUP（上一版固件，回滚源）
           |      BACKUP       |
0x080A0000 +-------------------+  256KB 扇区9-10  DOWNLOAD（下载暂存）
           |   固件包 ≤232KB    |
0x080DE000 +-------------------+  24KB          会话槽区（768槽×32B，断点续传）
...
**分区要点**
- RUN 尾部 `0x0805FFF8`：有效魔数 `0x4F54412E`；`0x0805FFFC`：版本号。
