...
```text
0x08000000 +-------------------+  64KB  扇区0-3   BOOT（启动加载器）
           |      BOOT         |
0x08010000 +-------------------+  832KB 扇区4-10  RUN（APP 运行区，方案B）
           |       RUN         |
...
**分区要点**
- RUN 尾部 `0x080DFFF8`：有效魔数 `0x4F54412E`；`0x080DFFFC`：版本号。
- **回滚源外移（方案B）**：内部 BACKUP/DOWNLOAD 取消，升级前将当前 RUN 全量
  备份到外部 Flash img_lib 分区（0x200000，头 4KB + 数据 832KB），PENDING
  回滚/自动修复由 BOOT 从外部备份槽恢复。
- **下载暂存外移**：外部 Flash ota_dl 分区（0x000000，2MB，单槽 1MB），
  YMODEM 兜底同样收包到外部槽。
- PARAM 双份冗余（slot0/slot1，间距 1KB），CRC-32 只覆盖 `crc32` 字段之前的数据（避免自引用）。
- PARAM 区 0x080E2000 起为 OTA 断点续传会话槽区（`magic 0x4F54414D + version + total + received + crc32`）。
- PARAM 区 0x080E1000 起 4KB 为 APP 网络配置 NVM 日志（`net ip` 持久化，
  32B 槽 ×128，追加写；BOOT 升级提交后失效全部会话槽防旧包续传）。
