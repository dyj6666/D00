...
/* ---------- 兼容层：替代官方 SYSTEM/sys.h，适配 D00 工程 ---------- */
...
#define SYS_GPIO_MODE_AF             GPIO_MODE_AF_PP
#define SYS_GPIO_MODE_OUT            GPIO_MODE_OUTPUT_PP
#define SYS_GPIO_OTYPE_PP            0
#define SYS_GPIO_SPEED_HIGH          GPIO_SPEED_FREQ_HIGH
#define SYS_GPIO_PUPD_PU             GPIO_PULLUP

static inline void sys_gpio_set(GPIO_TypeDef *port, uint16_t pin,
...
static inline void sys_gpio_af_set(GPIO_TypeDef *port, uint16_t pin,
                                   uint32_t af)
{
    GPIO_InitTypeDef g = {0};
    g.Pin = pin;
...
static inline void sys_gpio_pin_set(GPIO_TypeDef *port, uint16_t pin,
                                    uint8_t x)
{
...
{
    volatile uint32_t n = us * 84u;   /* ~168MHz 粗粒度微秒延时 */
    while (n--) {
    }
}
...
void lcd_bench(void);               /* 性能基准：清屏/填充/字符/字符串速率 */
