...
2. **弹出"固件已过时请更新"→ 点确认后报"不支持的主板架构"**（OpenART mini 必踩）
   - **原因**：OpenMV IDE 版本较新（2.6.7），逐飞出厂固件版本旧——IDE 检测到版本低就
     弹"固件过时"；若点**确认**会进入固件升级流程，而升级流程不认识逐飞定制主板
     （RT1064 架构，IDE 内置 boards.txt 只有标准 OpenMV 主板），于是报"不支持的主板架构"
   - **解决办法：弹窗点「取消/× 关闭」跳过升级**——过时只影响"升级固件"功能，
     **不影响连接、运行脚本、实时显示图像**（实测可用）
   - 如需真正升级固件：需从**逐飞官方渠道**获取 OpenART mini 专用固件与升级工具
     （官网资料区/公众号/淘宝客服），不能用 OpenMV IDE 通用升级
3. **画面黑屏 / 无图像**
