...
def derive_aes_key_from_uid(uid_hex):
    print(f"[诊断] 输入的 UID 字符串: {uid_hex}")
    uid_ints = [int(uid_hex[i:i+8], 16) for i in range(0, 24, 8)]
    uid_bytes = struct.pack('<III', *uid_ints)
    salt = b"OTA-AES-KEY-V1\x00"   # 16 字节
    print(f"[诊断] UID 字节 (hex): {uid_bytes.hex().upper()}")
    print(f"[诊断] 盐字节 (hex): {salt.hex().upper()}")
    sha_input = uid_bytes + salt
    print(f"[诊断] SHA256 输入 (hex): {sha_input.hex().upper()}")
    h = SHA256.new(sha_input)
    key = h.digest()
    print(f"[诊断] 完整 AES 密钥: {key.hex().upper()}")
    return key
...
if __name__ == '__main__':
    ...
    aes_key = derive_aes_key_from_uid(DEVICE_UID)

    encrypt_and_sign(firmware, secure_firmware, PRIVATE_KEY_HEX, aes_key.hex())
